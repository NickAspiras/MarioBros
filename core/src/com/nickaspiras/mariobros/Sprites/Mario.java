...
public class Mario extends Sprite {
    ...
    public Mario(PlayScreen screen){
        ...
        for(int j = 1; j < 4; j++){
            frames.add(new TextureRegion(screen.getAtlas().findRegion("big_mario"), j * 16, 0, 16, 32));
        }
        ...
        for(int k = 4; k < 6; k++){
            frames.add(new TextureRegion(screen.getAtlas().findRegion("little_mario"), k * 16, 0, 16, 16));
        }
    ...
    public void hit(Enemy enemy){
        if(enemy instanceof Turtle && ((Turtle)enemy).getCurrentState() == Turtle.State.STANDING_SHELL){
            ...
            if (marioIsBig) {
                ...
                MarioBros.manager.get("audio/music/GrassLandTheme.ogg", Music.class).stop();
